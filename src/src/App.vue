<template>
  <div id="app">
    <keep-alive>
      <v-head :alternum='alternuminfo' v-if="iSshow && !full" />
    </keep-alive>
  ...
  methods: {
    ...
    alarmInform() {
      ...
      if (this.$route.fullPath == '/login' || this.$route.fullPath == '/') {
        ...
        axios.getAlarmInform().then(res => {
          this.alternuminfo = Number(res.data.data.warning) + Number(res.data.data.critical)
        })
      }
    },
    // 播放音频
    ...
    getAibmsWebsocket() {
      this.$global.wsAibms.onmessage = res => {
        ...
        if (data) {
          if (data.alert) {
            if (data.alert.level == '2' || data.alert.level == '1') { console.log(data) }
            this.notifyAlert()
          }
        }
      }
