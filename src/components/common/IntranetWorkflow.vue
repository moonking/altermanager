...
import { setxmlname, randChar } from '../../utils/commonjs'
export default {
  name: 'Workflow',
  props: {
    activityData: {
    ...
    status: {
      type: String,
      default: () => {
        return ''// read:查看 ， edit:编辑 ，update:编辑
      }
    }
  },
  data () {
  ...
  methods: {
    ...
    exchangeName (name) {
      if (this.xmlStr) {
        this.xmlStr = setxmlname(this.xmlStr, this.ID, name)
        this.createNewDiagram(this.bpmnModeler, this.xmlStr)
      } else if (this.activityData.actXml) {
        this.activityData.actXml = setxmlname(
      ...
      if (newxml) {
        ...
        n.importXML(this.bpmnXmlStr, function (err) {
          ...
          if (_this.status === 'read') {
            ...
            let domList2 = document.getElementsByTagName('polyline')
            for (let dom2 of domList2) {
              dom2.onclick = function () {
                setTimeout(() => {
                  _this.changeExtend(2)
                }, 0)
              }
            }
            let domList = document.getElementsByClassName('djs-shape')
    ...
    clearInput () {
      ...
      if (
        ...
        document.getElementsByClassName('djs-direct-editing-content')[0].setAttribute('contenteditable', false)
        document.getElementsByClassName('djs-direct-editing-parent')[0].style.display = 'none'
        let domList = document.getElementsByClassName('djs-shape')
        for (let i = 0; i < domList.length; i++) {
          if (
            domList[i].className.baseVal.indexOf('djs-label-hidden') > -1
          ) {
            domList[i].className.baseVal = domList[i].className.baseVal.replace('djs-label-hidden', '')
          }
        }
      }
    },
    // 绑定事件已经回显数据
    getreturndata () {
      if (this.status == 'edit' || this.status == 'read') {
        if (this.status == 'read') {
          // 详情则不显示左侧工具栏
          document.querySelector('.djs-palette').style.display = 'none'
          // 如果时详情点击元素则不显示右侧的工具
          document.querySelector('.djs-overlay-container').style.display =
            'none'
        }
        let glist = document.getElementsByClassName('layer-base')[0]
        let _this = this
        glist.ondblclick = function (event) {
          if (
            event.target.parentElement.dataset.elementId.indexOf('Task') > -1
          ) {
            _this.showDiaLog = true
            _this.$emit('update:showDialog', _this.showDiaLog)
            _this.$emit(
              'update:ID',
    ...
    newnode () {
      let _this = this
      if (_this.status != 'read' && _this.status != 'edit') {
    ...
    // 修改右边提示框文字与数量
    changeExtend (n) {
      if (document.querySelector('.djs-context-pad')) {
        let ContextList = document.querySelector('.djs-context-pad').children

        for (let i = 0; i < ContextList.length; i++) {
          if (n === 1) {
            if (ContextList[0].children.length === 2) {
              ContextList[0].children[0].style.display = 'none'
              ContextList[0].children[1].setAttribute('title', '删除')
              ContextList[1].style.display = 'none'
              ContextList[2].style.display = 'none'
              continue
            }
            if (i == 0) {
              ContextList[i].children[3].style.display = 'none'
              ContextList[i].children[4].style.display = 'none'
              ContextList[i].children[0].setAttribute('title', '结束')
              ContextList[i].children[1].setAttribute('title', '并行网关')
              ContextList[i].children[2].setAttribute('title', '任务')
            }
            if (i == 1) {
              ContextList[i].children[0].style.display = 'none'
              ContextList[i].children[1].setAttribute('title', '删除')
            }
            if (i == 2) {
              ContextList[i].style.display = 'none'
            }
          } else if (n === 2) {
            if (ContextList[0].children.length === 1) {
              ContextList[0].children[0].setAttribute('title', '删除')
            }
            if (ContextList[0].children.length === 2) {
              ContextList[0].children[0].style.display = 'none'
              ContextList[0].children[1].setAttribute('title', '删除')
    ...
    setEncoded (link, name, data) {
      ...
      // 这里处理xml变化时图片消失
      this.viewxmlstages()
      // 任务框可输入input 需要的话注释下面方法 ，不需要的话打开下面方法
      this.$nextTick(() => {
        this.clearInput()
      })
    },
...
.djs-label-hidden .djs-visual /deep/ .djs-label{
