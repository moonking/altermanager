wrapper<template>
  <div class="container refreshContainer non-border non-shadow white-color aibms-bg">
    <div class="wrapper aibms-color-bg non-border non-shadow white-color">
        ...
        <div class="block-content" :class="{height0: !blockSwitch[1]}">
          ...
          >
            <el-form-item label="脚本名称：" prop="name">
              <el-input v-model="baseFormData.name" maxlength="20" :style="{width: '632px'}"></el-input>
            </el-form-item>
            ...
            <el-form-item label="标签云：">
              <div>
                ...
                <div v-if="selectTagList.length > 0" style="width: 632px;">
                  <div class="tags-box">
            ...
            <el-form-item label="脚本类型：" prop="toolType">
              <el-select v-model="baseFormData.toolType" :style="{width: '632px'}">
                <el-option
                  :label="ty.label"
                  :value="ty.value"
            ...
            <el-form-item label="脚本说明：">
              <el-input v-model="baseFormData.remarks" maxlength="150" :style="{width: '632px'}"></el-input>
            </el-form-item>
            <el-form-item label="执行工具：" prop="runTool">
              <el-select v-model="baseFormData.runTool" :style="{width: '632px'}">
                <el-option label="ansible" value="ansible"></el-option>
                <el-option label="puppet" value="puppet"></el-option>
              </el-select>
              <!-- <el-select v-model="baseFormData.runTool" :style="{width: '632px'}">
                <el-option
      ...
      <div class="block-item output-param">
        <div class="item-block-title paddingTop" @click="closeBlock(3)">
          <div class="item-block-title-mark"></div>
      ...
      <div class="block-item approve-info">
        <div class="item-block-title paddingTop" @click="closeBlock(4)">
          <div class="item-block-title-mark"></div>
      ...
      <div class="confirm-canseladd">
        <button
          class="cm-btn deep margin-right"
          :style="{marginRight: '100px'}"
          @click.stop="addTool"
        >保存</button>
        <span v-if="!currentId">
          <button
            class="cm-btn isoutline margin-right"
            :style="{marginRight: '100px'}"
            @click.stop="addTool(1)"
          >保存并发起审批</button>
        </span>
        <button class="cm-btn isoutline" @click="cancelAdd">取消</button>
      </div>
      <div class="clear"></div>
    </div>
        ...
        <el-col :span="24">
          <div class="tags-box">
            <div class="newBtn" @click="openTagDialog">
              <icon-svg icon-class="wenjianjia" class="middle-operation-color" />
              <span>创建标签分类</span>
            </div>
            <ul>
  ...
  beforeMount () {
    ...
    Promise.all([
      ...
      if (tagsListArr.data.code === 200) {
        let data = tagsListArr.data.data
        // 将数据解析生成对应的标签
        for (let key of data) {
          key.clickEdit = false
          key.typetag = 0
  ...
  methods: {
    ...
    deleteTagArr (tag, index) {
      this.$confirm('此操作将删除改类别及其下子标签, 是否继续?', '提示', {
        ...
        .then(() => {
          if (tag.grouptagId) {
            axios.deleteTag(tag.grouptagId, 1).then(res => {
              if (res.data.code === 200) {
                this.$notify({
                  type: 'success',
                  message: '删除成功'
    ...
    handleInputTag (tag) {
      if (tag.grouptagId) {
        ...
        if (tag.name) {
          ...
          axios.scriptTag(params).then(res => {
    ...
    updateTools (params, str) {
      axios.updateParams(params).then(res => {
    ...
    addNewTools (params, str, arr) {
      if (this.saveToolId) {
        this.approveTools(this.saveToolId)
      } else {
        axios.addNewScript(params).then(res => {
          if (res.data.code === 500) {
            this.$notify({
              type: 'warning',
              title: '提示',
            ...
            this.saveToolId = res.data.data.toolId
            this.version = res.data.data.version
            if (str === 1) {
              this.approveTools(res.data.data.toolId, res.data.data.version)
            } else {
              //  if(str === 0) {
              //   this.approveTools(res.data.data, 0);
              this.$notify({
                type: 'success',
                title: '提示',
                message: res.data.message
