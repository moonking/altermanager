  ...
  <div class="container refreshContainer aibms-color-bg non-border non-shadow white-color">
      ...
      >
        ...
        >
          ...
          <el-form-item label="优先级：" prop="priority">
            <el-select v-model="baseInfoForm.level" :style="{width: '632px'}" placeholder="请选择优先级">
              <el-option
    ...
    <div class="approve-info">
      <div class="item-block-title paddingTop" @click="switchBlock(2)">
        <div class="item-block-title-mark"></div>
        <span class="item-block-title-font">任务流水线</span>
        <i
          class="icons el-icon-arrow-right"
      ...
      >
        <div style="overflow: visible;padding-left: 52px;">
    ...
    <div class="approve-info">
     ...
     <h3>
      ...
      <div class="item-block-title paddingTop" @click="switchBlock(3)">
        <div class="item-block-title-mark"></div>
      ...
      <div
        class="block-content item-block-content"
        :style="{height: blockSwitch[3] ? 'auto' : '0' }"
      >
        <el-form
          :model="baseInfoForm"
          :rules="baseInfoFormRules"
        ...
        >
          <el-form-item label="审批人：" prop="approvalids">
            <user-selector
              :selectedList.sync="baseInfoForm.approvalids"
              :readonly="status === 'read'"
              :candidates="userList"
            ></user-selector>
          </el-form-item>
          <el-form-item label="抄送人：">
            <user-selector
              :selectedList.sync="baseInfoForm.sendcopyids"
              :readonly="status === 'read'"
              :candidates="userList"
            ></user-selector>
          </el-form-item>
    ...
    <div class="save-and-cancel dialog-footer-form" v-if="status !== 'read'">
      <el-button type="primary" @click="handleSave" style="margin-right: 200px">保存</el-button>
    ...
    />
    <!-- <el-dialog title="编辑节点" :visible.sync="showDialog" center>
      <el-form
        ref="pipelineForm"
        :model="pipelineForm"
        label-width="100px"
      ...
      >
        <el-form-item label="节点名称: " class="disinlne" prop="actNodeJobId">
          <el-select
        ...
        <el-form-item label="任务依赖: " class="disinlne" prop="relyOn">
          <el-select
            v-model="pipelineForm.relyOn"
            placeholder="请选择任务依赖"
            style="width:55%"
            filterable
            clearable
    ...
    <el-dialog title="选择任务" width="70%" :visible.sync="selectTaskDialogVisible" center>
      <select-task
        ...
        :systemId="baseInfoForm.systemId"
        :taskType="taskTypes.deploy"
      ></select-task>
      <div slot="footer" class="dialog-footer-form" :style="{textAlign: 'center'}">
...
export default {
  created () {
    this.getAllSelectList().then(res => {
      if (res[0] && res[0].data.success) {
        this.systemList = res[0].data.data.result
      }
      if (res[1] && res[1].data.success) {
  ...
  data () {
    var validateTime = (rule, value, callback) => {
      if (value === '') {
        return callback(new Error('请输入预计发布时长，单位min'))
      } else {
    ...
    return {
      ...
      activityData: {},
      actNodeName: '',
      blockSwitch: { '1': true, '2': true, '3': false },
      // 已选择任务流水线
      selectedTasks: [],
      // 业务系统待选列表
      systemList: [],
  ...
  methods: {
    ...
    //   this.$set(this.pipelineForm, "relyOn", "");
    // },
    // 获取xml
    getXml (data) {
    ...
    fetchTaskData () {
      ...
      axios.releaseTaskDetail(id).then(res => {
        if (res.data.success) {
          ...
          /**
           * 任务流水线数据处理，只需要四个属性
           * jobId, jobtaskSort, jobtaskStrategy, name
           */
    ...
    dataTransform (data) {
      ...
      let approvalids = data.approvalids ? JSON.parse(data.approvalids) : []
      let sendcopyids = data.sendcopyids ? JSON.parse(data.sendcopyids) : []
      ...
      for (let n = 0; n < approvalids.length; n++) {
        approvalids[n] = this.getUserInfoFromUserList(approvalids[n].userId)
      }
      for (let p = 0; p < sendcopyids.length; p++) {
        sendcopyids[p] = this.getUserInfoFromUserList(sendcopyids[p].userId)
      }
      result.approvalids = approvalids
      result.sendcopyids = sendcopyids
    ...
    handleSave () {
      ...
      data.approvalids = this.userInfoTransform(data.approvalids)
      if (data.approvalids.length === 0) {
        this.$notify({
          type: 'error',
      ...
      data.sendcopyids = this.userInfoTransform(data.sendcopyids)
      data.actXml = this.actXml || data.actXml
      data.NodeParams = this.NodeParams || data.releaseNodeparams
      this.FormValidate()
        .then(() => {
          if (this.status === 'update') {
            this.updateDeployReleaseTask(data)
          } else if (this.status === 'create') {
            this.addDeployReleaseTask(data)
          }
        })
        .catch(() => { })
    },
    ...
    // 表单统一验证
    FormValidate () {
      return this.$refs.baseInfoForm.validate()
    },
    // 选择任务流水线
    selectTaskPipeline () {
    ...
    confirmSelectedTasks () {
      this.selectedTasks = this.$store.state.tempDeployCheckTaskList
  ...
  computed: {
    pickerOptionsStart () {
      return {
        disabledDate: time => {
          if (this.baseInfoForm.planEndTime === null) {
            return time.getTime() < new Date().getTime() - 1000 * 3600 * 24
          }
          // 若时间有误差，可加减一天
          return (
            time.getTime() >
            new Date(this.baseInfoForm.planEndTime).getTime() ||
    ...
    pickerOptionsEnd () {
      return {
        disabledDate: time => {
          if (this.baseInfoForm.planStartTime === null) {
            return time.getTime() < new Date().getTime() - 1000 * 3600 * 24
          }
          return (
            time.getTime() < new Date(this.baseInfoForm.planStartTime).getTime()
          )
        }
      }
    },
    ...
    isShowDialog: {
      handler (newValue, oldValue) {
        if (newValue) {
          ...
          this.NodeParams.forEach(item => {
            if (item.actNodeId === this.ID) {
              this.actNodeName = item.actNodeName
              bl = true
            }
          })
          if (!bl) this.actNodeName = ''
        }
...
/* .header-title span .title-img {
  height: 12px;
  width: 12px;
  background-image: url('../../../../static/img/reablue.png');
} */
