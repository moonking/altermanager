<template>
  <div class="task-timeline">
        ...
        <div class="timeline">
          <div class="direction-h clear-fix">
            <div :class="['h-left', 'clear-fix', {hidden: !isOdd(index)}]">
              <div class="step-text" :style="{marginRight: '10px',textAlign: 'right'}">{{getStepTaskNum(index)}}</div>
              <div class="h-line"></div>
            </div>
            <div class="line-icon"><i class="el-icon-time"></i></div>
    ...
    <draggable v-model="tempCheckList" draggable=".drag-item">
      <div :class="['task-item','clear-fix', {'drag-item': !readonly} ]" v-for="(item, index) in tempCheckList" :key="index">
        <div :class="['task-info-left', {hidden: !isOdd(index)}]">
          <div class="info-box">
            ...
            <div class="task-dependency">
              任务依赖：
              <el-select v-model="item.jobTaskStrategy" size="mini" :style="{width: '126px'}" :disabled="!isOdd(index) || readonly">
                <el-option
...
.task-info-left,.task-info-right {
  width: 50%;
  float: left;
}
.info-box {
